...
X_size = [x[i/2] if i % 2 == 0 else (x[i-1/2] + x[i+1/2])/2 for i in range(size)]
X_t_X_size = X_size.T * X_size
Y_signal_size = [Y_signal[i/2] if i % 2 == 0 else (Y_signal[i-1/2] + Y_signal[i+1/2])/2 for i in range(size)]

for i in range(size):
    matr_1 = np.matmul(X_size[i], X_t_X_size)
    matr_2 = np.matmul(matr_1, np.transpose(X_size[i]))
    a_95[i] = template.format(
        Y_signal_size[i] - kvantil1 * norm_E_sr * (np.sqrt(matr_2)) / np.sqrt(n - m - 1))
    b_95[i] = template.format(
        Y_signal_size[i] + kvantil1 * norm_E_sr * (np.sqrt(matr_2)) / np.sqrt(n - m - 1))
...
for i in range(l+1):
    pi[i] = abs(sp.stats.norm.cdf(t[i+1]/σ_sr) -
                sp.stats.norm.cdf(t[i]/σ_sr))
